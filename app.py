...
app = Flask(__name__)
app.secret_key = "change-this-secret-key-in-production"  # needed for session

# --- CONFIGURATION ---
MAX_ALLOWED_FAILS = 4
LOCKOUT_DURATION = 240
...
# ============================================================
# SHARED STYLES  (used by both pages -> keeps things DRY)
# ============================================================
SHARED_STYLES = """
<style>
    :root {
...
# PAGE 1 : LOGIN (HOME)
# ============================================================
LOGIN_TEMPLATE = SHARED_STYLES + """
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Sign In • Brute-force Detection System</title>
    <style>
        .login-wrapper {
        ...
        .login-card p.subtitle {
            color: var(--text-sub); font-size: 0.85rem; margin-bottom: 1.5rem;
        }
    </style>
</head>
<body>
    ...
    <div class="login-wrapper">
        <div class="card login-card">
            ...
            <p class="subtitle">Brute-force Detection System</p>
...
# PAGE 2 : DASHBOARD
# ============================================================
DASHBOARD_TEMPLATE = SHARED_STYLES + """
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dashboard • Brute-force Detection System</title>
    ...
    <style>
        ...
        .tag { padding: 0.2rem 0.6rem; border-radius: 6px; font-size: 0.75rem; font-weight: 600; display: inline-block; }
        ...
        .nav-link { color: var(--text-sub); text-decoration: none; font-size: 0.85rem; margin-right: 1rem; }
...
<body>
    ...
    <div class="dashboard-header">
        <div>
            ...
            <p style="font-size: 0.85rem; color: var(--text-sub);">
                Real-time Threat Monitoring &amp; Dynamic Blocklist
            </p>
        </div>
    ...
    <div class="grid-layout">
        <!-- LEFT: Manual IP Override -->
        <div class="card">
            <h4 style="font-size: 0.95rem; margin-bottom: 0.75rem;">Instant Manual IP Override</h4>
            <div class="form-group">
    ...
    <script>
        ...
        function initMap() {
            ...
            L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png?key=cb1_3j8z_1_6ab89a51f443b1de5e682f9f', {
...
# --- IP UNMASKING & GEOLOCATION (unchanged) ---
def get_real_ip(req):
...
def get_ip_metadata(ip):
    if ip in ['127.0.0.1', 'localhost', '::1'] or ip.startswith(('10.', '192.168.', '172.')):
        ...
        return {"country": loc["country"], "lat": loc["lat"], "lon": loc["lon"], "rep_score": 88}
    try:
        ...
        with urllib.request.urlopen(req, timeout=2) as response:
            ...
            if data.get('status') == 'success':
                return {
                    ...
                    "rep_score": int(np.random.uniform(65, 99))
                }
    except Exception:
        pass
...
@app.route('/dashboard')
def dashboard():
    """Dashboard page (protected by simple session check)."""
...
# --- Remaining API endpoints are unchanged ---
...
def get_history():
    ...
    for item in ip_history_logs:
        ...
        formatted_history.append({**item, "is_active": active})
    active_blocks_count = sum(1 for ip in blocked_ips if now < blocked_ips[ip]['unblock_time'])
    return jsonify({"history": formatted_history, "active_blocks_count": active_blocks_count})
